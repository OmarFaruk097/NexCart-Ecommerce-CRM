...
const SupportSystem = () => {
  ...
  useEffect(() => {
    fetch('https://nexcart-ecommerce-crm.onrender.com/api/crm/support')
      .then(res => res.json())
      .then(data => {
        setTickets(data);
        setLoading(false);
  ...
  const handleRespond = async (ticketId, currentStatus) => {
    ...
    try {
      const response = await fetch(`https://nexcart-ecommerce-crm.onrender.com/api/crm/support/${ticketId}`, {
        method: 'PUT',
